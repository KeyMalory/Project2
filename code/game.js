...
var actorChars = {
  ...
  "=": Lava, "|": Lava, "v": Lava,  // ;lava bits that move if add more make sure to add comma
  "m": Meow
};
  ...
  for (var y = 0; y < this.height; y++) {
    ...
    for (var x = 0; x < this.width; x++) {
      ...
      // Because there is a third case (space ' '), use an "else if" instead of "else"
        ...
        //if want new non moving stuff add here, vid 1 5min for details
...
// constructor + act method, constructor:
function Meow (pos) {
  this.basePos = this.pos = pos.plus(new Vector(.1,.2));
  // off set geometry size ish
  this.size = new Vector(.4,.4);
  // not new grid space, but diff size
  this.wobble = Math.random() * Math.PI * 2.5;
  //random # betw 1 and 0, wiggly
}
Meow.prototype.type = 'meow';
...
Coin.prototype.act = function(step) {
  ...
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
   // geometry and trig i hateeee it. i Hate it.
};

var wobbleSpeed = 2
var wobbleDist = .3

Meow.prototype.act = function(step) {
  this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * wobbleDist;
  this.pos = this.basePos.plus(new Vector(.3, wobblePos));
};
